...
#show raw: set text(font: ("Fira Code", "Source Han Serif SC"))
...
- 点击电梯内任意数字按键发出任务请求，电梯在正常状态下将移动至请求楼层；
...
其中 `Elevator` 包含了单个电梯的界面与运行逻辑，用于处理单电梯的任务调度与处理；`Controller` 由 `Elevator` 数组与电梯外按键组成，用于响应电梯外按键的请求，并将新任务分配给某一电梯。
